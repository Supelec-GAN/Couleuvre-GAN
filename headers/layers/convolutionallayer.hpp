...
class ConvolutionalLayer : public NeuronLayer
{
    public:
         ...
         * La matrice de poids est de dimension outputSize x inputSize
         */
                        ConvolutionalLayer(unsigned int tailleImg, unsigned int nbChannels, std::vector<Eigen::MatrixXf> weight, std::function<float(float)> activationF= Functions::sigmoid(10.f));
